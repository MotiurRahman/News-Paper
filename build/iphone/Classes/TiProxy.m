...
//Rare exceptions to indicate a bug in the _newspaper code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_newspaper = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_newspaper]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.newspaper.module"
